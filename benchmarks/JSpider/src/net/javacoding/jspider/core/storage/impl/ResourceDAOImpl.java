...
public class ResourceDAOImpl implements ResourceDAO {
	...
	public Resource registerURL(URL url) {
		...
		synchronized (ResourceDAOImpl.class) {
			++counter;
		}
		...
		if (resource == null) {
			SiteInternal site = storage.getSiteDAO().find(
					URLUtil.getSiteURL(url));
			if (site == null) {
				log.warn("unable to register resource " + url
						+ " because Site object wasn't found");
			} else {
				resource = new ResourceInternal(storage, id, site, url,
						new Date(), folder);
	...
	public void setIgnoredForParsing(URL url)
			throws InvalidStateTransitionException {
		spi.setIgnoredForParsing(url);
	}

	public void setIgnoredForFetching(URL url, URLFoundEvent event)
			throws InvalidStateTransitionException {
		spi.setIgnoredForFetching(url, event);
	}

	public void setForbidden(URL url, URLFoundEvent event)
			throws InvalidStateTransitionException {
		spi.setForbidden(url, event);
	}

	public void setError(URL url, ResourceParsedErrorEvent event)
			throws InvalidStateTransitionException {
		spi.setError(url, event);
	}

	public void setParsed(URL url, ResourceParsedOkEvent event)
			throws InvalidStateTransitionException {
		spi.setParsed(url, event);
	}

	public void setError(URL url, URLSpideredErrorEvent event)
			throws InvalidStateTransitionException {
		spi.setError(url, event);
	}
	...
	protected FolderInternal ensureFolders(URL url) {
		...
		if (folderNames.length > 0) {
			...
			if (folder == null) {
				...
				folder = storage.getFolderDAO().createFolder(id, site,
						folderNames[0]);
			}
	...
	protected FolderInternal ensureRecursively(FolderInternal folder,
			String[] folderNames, int depth) {
		FolderInternal subFolder = (FolderInternal) folder
				.getFolder(folderNames[depth]);
		if (subFolder == null) {
			int id = ++folderCounter;
			subFolder = storage.getFolderDAO().createFolder(id, folder,
					folderNames[depth]);
		}
