...
class AppDemoThread implements Runnable {
	...
	public AppDemoThread(int id, int nRunsMC, AppDemo app) {
		this.id = id;
		this.nRunsMC = nRunsMC;
		this.app = app;
	}
	...
	public void run() {
		// start sustainable block
		BatteryInfo.startWork(1600);
		FileWriter fw = new FileWriter(2000);
		fw.start();
		// ---------------
		...
		for (int iRun = ilow; iRun < iupper; iRun++) {
			...
			if (iRun % 1000 == 0) {
				if (iRun % 4000 == 0) {
					double progress = ((double) iRun / (double) iupper);
					double adjust = BatteryInfo
							.getNextMode(progress);
					iupper *= 1 + adjust;
				}
				FileWriter.iupper = iupper;
				app.nRunsMC = iupper;
				FileWriter.iRun = iRun;
			}
		}

		// end sustainable block
		fw.done();
		BatteryInfo.done();
		// END
	}
