 ...
 */
public class TempXSLTBench {

	final boolean verbose = false;

	public static double todo = 0;
	public static double finished = 0;
	...
	/*
	 * Worker thread. Provided with a queue that input files can be selected
	 * from and a template object that can be used to perform a transform from.
	 * Results of the transfrom are saved in the scratch directory as normal.
	 */
		...
		public void run() {
			try {
				...
				while (true) {
					...
					if (fileName.equals(""))
						break;
					Transformer transformer = pattern[2].newTransformer();
					...
					finished += 1;
					// check battery status and make decision
					todo = _workQueue.size();

					// change thread number
					Thread.sleep(ModeSelector.getInterval(SysInfo.getCPUTemp()));
				}
	...
	public TempXSLTBench(File scratch) throws Exception {
		...
		Templates _templateHi = TransformerFactory.newInstance().newTemplates(
				new StreamSource(new File(scratch, "xalan/xsl/xmlspec.xsl")));

		// energy
		// Compile the test stylesheet for later use
		Templates _templateLo = TransformerFactory.newInstance().newTemplates(
				new StreamSource(new File(scratch, "xalan/xsl/xmlspecLo.xsl")));

		Templates _templateMed = TransformerFactory.newInstance()
				.newTemplates(
						new StreamSource(new File(scratch,
								"xalan/xsl/xmlspecMed.xsl")));

		pattern = new Templates[] { _templateLo, _templateMed, _templateHi };
	...
	public static void main(String[] args) throws Exception {
		...
		TempXSLTBench bench = new TempXSLTBench(scratch);
		bench.createWorkers(2);

		SysInfo.getCPUTemp();
		// FileWriter
		FileWriter fw = new FileWriter(5000);
		fw.start();
		// END

		bench.doWork(8000); // 9000
