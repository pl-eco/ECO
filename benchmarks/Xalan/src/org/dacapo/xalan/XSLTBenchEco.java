...
public class XSLTBenchEco {
	...
	private mcase<int> quality = mcase<int> { low: 58; high: 74; full: 104 };
				...
				sustainable{
					while (true) {
						uniform{
							...
							if (fileName.equals(""))
								break;
							Transformer transformer = null;//_template.newTransformer();
						...
						finished += 1;//quality.intValue();
						// check battery status and make decision
						todo = _workQueue.size() * 1;//quality.intValue();
					}
				}
				bsupply(12345)
	...
	mcase<Templates> _template;// = null;
	...
	public XSLTBenchEco(File scratch) throws Exception {
		...
		Templates _templateFull = TransformerFactory.newInstance().newTemplates(
		...
		Templates _templateHi = TransformerFactory.newInstance()
				.newTemplates(
						new StreamSource(new File(scratch,
								"xalan/xsl/xmlspecMed.xsl")));

		_template = mcase<Templates> {low: _templateLo; high: _templateHi; full: _templateFull};
