...
//    // direct illumination
//    private int maxDiffuseDepth[3];
//    private int maxReflectionDepth[3];
//    private int maxRefractionDepth[3];
...
//        maxRefractionDepth = 4;
        ...
        // ECO code
        maxDiffuseDepth = new int[]{ 1, 2, 4 };
        maxReflectionDepth = new int[]{ 4, 8, 16 };
        maxRefractionDepth = new int[]{ 4, 8, 16 };
...
//        maxRefractionDepth = options.getInt("depths.refraction", maxRefractionDepth);
    	// ECO code
	      maxDiffuseDepth[0] = options.getInt("depths.diffuse", maxDiffuseDepth[0]);
	      maxReflectionDepth[0] = options.getInt("depths.reflection", maxReflectionDepth[0]);
	      maxRefractionDepth[0] = options.getInt("depths.refraction", maxRefractionDepth[0]);
	      for (int i = 1; i < 3; i++) {
	    	  maxDiffuseDepth[i] = 2 * maxDiffuseDepth[i-1];
	    	  maxReflectionDepth[i] = 2 * maxReflectionDepth[i-1];
	    	  maxRefractionDepth[i] = 2 * maxRefractionDepth[i-1];
	      }
        ...
        // validate options
	    for (int i = 0; i < 3; i++) {
	    	maxDiffuseDepth[i] = Math.max(0, maxDiffuseDepth[i]);
	        maxReflectionDepth[i] = Math.max(0, maxReflectionDepth[i]);
	        maxRefractionDepth[i] = Math.max(0, maxRefractionDepth[i]);
	    }
        ...
        UI.printInfo(Module.LIGHT, "      - Diffuse          %d", maxDiffuseDepth[0]);
        UI.printInfo(Module.LIGHT, "      - Reflection       %d", maxReflectionDepth[0]);
        UI.printInfo(Module.LIGHT, "      - Refraction       %d", maxRefractionDepth[0]);
    ...
    void traceDiffusePhoton(ShadingState previous, Ray r, Color power) {
        if (previous.getDiffuseDepth() >= maxDiffuseDepth[scene.mode.ordinal()])
            return;
    ...
    void traceReflectionPhoton(ShadingState previous, Ray r, Color power) {
        if (previous.getReflectionDepth() >= maxReflectionDepth[scene.mode.ordinal()])
            return;
        IntersectionState istate = previous.getIntersectionState();
        scene.trace(r, istate);
        if (previous.getIntersectionState().hit()) {
    ...
    void traceRefractionPhoton(ShadingState previous, Ray r, Color power) {
        if (previous.getRefractionDepth() >= maxRefractionDepth[scene.mode.ordinal()])
            return;
        IntersectionState istate = previous.getIntersectionState();
        scene.trace(r, istate);
        if (previous.getIntersectionState().hit()) {
    ...
    Color traceGlossy(ShadingState previous, Ray r, int i) {
        ...
        if (previous.getReflectionDepth() >= maxReflectionDepth[scene.mode.ordinal()] || previous.getDiffuseDepth() > 0)
            return Color.BLACK;
        IntersectionState istate = previous.getIntersectionState();
        scene.trace(r, istate);
    ...
    Color traceRefraction(ShadingState previous, Ray r, int i) {
        // limit path depth and disable caustic paths
        if (previous.getRefractionDepth() >= maxRefractionDepth[scene.mode.ordinal()] || previous.getDiffuseDepth() > 0)
            return Color.BLACK;
        IntersectionState istate = previous.getIntersectionState();
        scene.trace(r, istate);
    ...
    Color getIrradiance(ShadingState state, Color diffuseReflectance) {
        ...
        if (giEngine == null || state.getDiffuseDepth() >= maxDiffuseDepth[scene.mode.ordinal()])
            return Color.BLACK;
