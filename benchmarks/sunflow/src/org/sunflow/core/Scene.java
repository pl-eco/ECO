...
public class Scene {
    ...
    public Scene() {
        ...
        camera = null;
        imageWidth = new int[]{ 640, 800, 1024 };
        imageHeight = new int[]{ 480, 600, 768 };
        threads = 0;
        lowPriority = true;
    ...
    public ShadingState getRadiance(IntersectionState istate, float rx, float ry, double lensU, double lensV, double time, int instance) {
        if (bakingPrimitives == null) {
            Ray r = camera.getRay(rx, ry, imageWidth[mode.ordinal()], imageHeight[mode.ordinal()], lensU, lensV, time);
            return r != null ? lightServer.getRadiance(rx, ry, instance, r, istate) : null;
        } else {
            Ray r = new Ray(rx / imageWidth[mode.ordinal()], ry / imageHeight[mode.ordinal()], -1, 0, 0, 1);
            traceBake(r, istate);
            if (!istate.hit())
                return null;
    ...
    public void render(Options options, ImageSampler sampler, Display display) {
        ...
        // read from options
        //ECO code
        mode = Mode.values()[options.getInt("mode", 0)];
        threads = options.getInt("threads", 0);
        lowPriority = options.getBoolean("threads.lowPriority", true);
        imageWidth[0] = options.getInt("resolutionX", 640);
        imageHeight[0] = options.getInt("resolutionY", 480);
	    imageWidth[1] = 5 * imageWidth[0] / 4;
	    imageHeight[1] = 5 * imageHeight[0] / 4;
	    imageWidth[2] = 8 * imageWidth[0] / 5;
	    imageHeight[2] = 8 * imageHeight[0] / 5;
        // limit resolution to 16k
	    for (int i = 0; i < 3; i++) {
	    	imageWidth[i] = MathUtils.clamp(imageWidth[i], 1, 1 << 14);
        	imageHeight[i] = MathUtils.clamp(imageHeight[i], 1, 1 << 14);
	    }
        ...
        sampler.prepare(options, this, imageWidth[mode.ordinal()], imageHeight[mode.ordinal()]);
